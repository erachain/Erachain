...
import static org.erachain.core.item.assets.AssetTypes.AS_OUTSIDE_GOODS;
...
public class TestRecSendMovable {
    ...
    private void init(boolean withIssue) {
        ...
        asset = new AssetVenture(itemAppData, deliver, "aasdasd", icon, image, "asdasda", AS_INSIDE_ASSETS, 8, 50000l);
        // set SCALABLE assets ++
        asset.insertToMap(db, BlockChain.AMOUNT_SCALE_FROM);
        asset.insertToMap(db, 0l);

        assetMovable = new AssetVenture(itemAppData, producer, "movable", icon, image, "...", AS_OUTSIDE_GOODS, scale, 500l);
    ...
    public void process_Movable_Asset() {
        ...
        r_Send = new RSend(producer, exLink, DAPP, FEE_POWER, recipient, keyMovable, BigDecimal.valueOf(1000),
        ...
        r_Send = new RSend(producer, exLink, DAPP, FEE_POWER, recipient, keyMovable, BigDecimal.valueOf(5),
                "", null, new byte[]{1}, new byte[]{1},
                timestamp++, 0l);
    ...
    public void validate_R_Send_Movable_Asset() {
        ...
        //CREATE ASSET TRANSFER
        r_Send = new RSend(producer, exLink, DAPP, FEE_POWER, spender, assetKey, BigDecimal.valueOf(1000),
                "", null, new byte[]{1}, new byte[]{1},
                ++timestamp, 0l);
        r_Send.setDC(db, Transaction.FOR_NETWORK, 1, 1, true);
        assertEquals(r_Send.isValid(Transaction.FOR_NETWORK, txFlags), Transaction.NO_BALANCE);

        r_Send = new RSend(producer, exLink, DAPP, FEE_POWER, spender, assetKey, BigDecimal.valueOf(50),
                "", null, new byte[]{1}, new byte[]{1},
                ++timestamp, 0l);
        r_Send.setDC(db, Transaction.FOR_NETWORK, 1, 2, true);
        assertEquals(r_Send.isValid(Transaction.FOR_NETWORK, txFlags), Transaction.VALIDATE_OK);
        ...
        r_Send = new RSend(
                ...
                prop1_backward,
                prop2,
                deliver, exLink, DAPP, FEE_POWER, spender, assetKey, BigDecimal.valueOf(-100),
                "", null, new byte[]{1}, new byte[]{1},
                ++timestamp, 0l);
        ...
        r_Send = new RSend(
                ...
                prop1_backward,
                prop2,
                deliver, exLink, DAPP, FEE_POWER, producer, assetKey, BigDecimal.valueOf(-10),
                "", null, new byte[]{1}, new byte[]{1},
                ++timestamp, 0l);
        ...
        r_Send = new RSend(
                ...
                prop1_backward,
                prop2,
                spender, exLink, DAPP, FEE_POWER, deliver, assetKey, BigDecimal.valueOf(-10),
                "", null, new byte[]{1}, new byte[]{1},
                ++timestamp, 0l);
        r_Send.setDC(db, Transaction.FOR_NETWORK, 1, 2, true);
        assertEquals(r_Send.isValid(Transaction.FOR_NETWORK, txFlags), Transaction.VALIDATE_OK);
