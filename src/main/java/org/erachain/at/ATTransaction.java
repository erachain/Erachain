...
public class ATTransaction {
    ...
    private final int BASE_SIZE = 2 * ATConstants.AT_ID_SIZE + 8 + 4 + 4 + 4;
    private byte[] senderId = new byte[ATConstants.AT_ID_SIZE];
    private byte[] recipientId = new byte[ATConstants.AT_ID_SIZE];
    private long key;
    private long amount;
    ...
    ATTransaction(byte[] senderId, byte[] recipientId, long key, long amount, byte[] message) {
    ...
    public ATTransaction(int blockHeight, int seq, byte[] senderId,
    ...
    public static ATTransaction fromBytes(byte[] data) {
        ...
        int seq = bf.getInt();
        byte[] senderId = new byte[ATConstants.AT_ID_SIZE];
        byte[] recipientId = new byte[ATConstants.AT_ID_SIZE];
        ...
        return new ATTransaction(blockHeight, seq, senderId, recipientId, key, amount, message);
    ...
    public byte[] toBytes() {
        ...
        bf.putInt(seq);
        bf.put(senderId);
        bf.put((recipientId != null) ? recipientId : new byte[ATConstants.AT_ID_SIZE]);
        bf.putLong(key);
        bf.putLong(amount);
    ...
    public boolean equals(Object otherObject) {
        if (otherObject instanceof ATTransaction) {
            ATTransaction otherAtTx = (ATTransaction) otherObject;
