...
public class DBRocksDBTableTransaction<K, V> extends DBRocksDBTable<K, V> implements Transacted {
    ...
    public DBRocksDBTableTransaction(Byteable byteableKey, Byteable byteableValue, String NAME_TABLE, List<IndexDB> indexes, DBASet dbaSet) {
        this(byteableKey, byteableValue, NAME_TABLE, indexes, RocksDbSettings.getDefaultSettings(),
                new WriteOptions().setSync(true).setDisableWAL(false),
                new ReadOptions(), dbaSet);
    }

    public DBRocksDBTableTransaction(String NAME_TABLE) {
    ...
    public void openSource() {
        dbSource = new RocksDbDataSourceOptTransaction(this.root, NAME_TABLE, indexes,
                (OptimisticTransactionDB) dbSource.getDbCore(), dbSource.getColumnFamilyHandles(), writeOptions, readOptions);
    }
