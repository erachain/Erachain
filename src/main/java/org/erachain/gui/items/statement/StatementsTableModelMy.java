...
public class StatementsTableModelMy extends AbstractTableModel implements Observer {
    ...
    private List<Transaction> read_Statement() {
        ...
        for (Account account : Controller.getInstance().getAccounts()) {
            transactions.addAll(DCSet.getInstance().getTransactionFinalMap().getTransactionsByAddressAndType(account.getAddress(), Transaction.SIGN_NOTE_TRANSACTION, 0, 0));//.SEND_ASSET_TRANSACTION, 0));
        }
