...
public class AccountTransactionsTable extends JTable implements Observer {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(AccountTransactionsTable.class);
    ...
    public AccountTransactionsTable() {
        ...
        for (Account account : Controller.getInstance().getAccounts()) {
            transactions.addAll(dcSet.getTransactionFinalMap().getTransactionsByAddressAndType(account.getAddress(), Transaction.SEND_ASSET_TRANSACTION, 0, 0));
        }
        ...
        for (Transaction messagetx : transactions) {
            ...
            if (!is) {
                addMessage(messageBufs.size(), (RSend)messagetx, null);
            }
        }
        ...
        copyMessage.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                ...
                AccountTransactionsTable invokerAsJComponent = (AccountTransactionsTable) invoker;
        ...
        this.addMouseListener(new MouseAdapter() {
            public void mouseClicked(MouseEvent e) {
                if (e.getClickCount() == 2) {
                    AccountTransactionsTable tableModelparent = (AccountTransactionsTable) e.getSource();
    ...
    public void Search_Accoutnt_Transaction_From_Asset(Account account, AssetCls asset) {
        ...
        transactions.clear();
        if (account != null) {
            transactions.addAll(dcSet.getTransactionFinalMap().getTransactionsByAddressAndType(account.getAddress(), Transaction.GENESIS_SEND_ASSET_TRANSACTION, 0, 0));
            transactions.addAll(dcSet.getTransactionFinalMap().getTransactionsByAddressAndType(account.getAddress(), Transaction.SEND_ASSET_TRANSACTION, 0, 0));
        }
    ...
    private void addMessage(int pos, Transaction transaction, Account account) {
        ...
        if (transaction.getType() == Transaction.GENESIS_SEND_ASSET_TRANSACTION) {
            ...
            RSend messagetx = (RSend) transaction;
            messageBufs.add(pos, new MessageBuf(
                    messagetx.getData(),
