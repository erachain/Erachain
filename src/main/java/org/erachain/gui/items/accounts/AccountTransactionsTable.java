...
public class AccountTransactionsTable extends JTable implements Observer {
    ...
    public void Search_Accoutnt_Transaction_From_Asset(Account account, AssetCls asset) {
        ...
        transactions.clear();
        if (account != null) {
            transactions.addAll(dcSet.getTransactionFinalMap().getTransactionsByAddressAndType(account.getShortAddressBytes(),
                    Transaction.GENESIS_SEND_ASSET_TRANSACTION, 0, 0));
            transactions.addAll(dcSet.getTransactionFinalMap().getTransactionsByAddressAndType(account.getShortAddressBytes(),
                    Transaction.SEND_ASSET_TRANSACTION, 0, 0));
        }
