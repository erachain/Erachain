...
public class MailsTransactionsTable extends JTable implements Observer {

    private static final Logger LOGGER = LoggerFactory.getLogger(MailsTransactionsTable.class);
    ...
    public MailsTransactionsTable(Boolean incoming) {
                ...
                if (messagetx.getAssetKey() == 0) {
                    for (Account account1 : Controller.getInstance().getWalletAccounts()) {
                        ...
                        if (a.getRecipient().getAddress().equals(account1.getAddress()) && incoming) {
                            addMessage(messageBufs.size(), (RSend) messagetx, null, incoming);
                        }
        ...
        copyMessage.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                ...
                MailsTransactionsTable invokerAsJComponent = (MailsTransactionsTable) invoker;
        ...
        this.addMouseListener(new MouseAdapter() {
            public void mouseClicked(MouseEvent e) {
                if (e.getClickCount() == 2) {
                    MailsTransactionsTable tableModelparent = (MailsTransactionsTable) e.getSource();
    ...
    public void Search_Accoutnt_Transaction_From_Asset(Account account, AssetCls asset) {
        ...
        for (Transaction messagetx : transactions) {
            ...
            if (asset.getKey() == messagetx.getAssetKey()) {
                ...
                if (!is) {
                    addMessage(messageBufs.size(), (RSend) messagetx, account, incoming);
                }
            }
        }
    ...
    public synchronized void syncUpdate(Observable o, Object arg) {
        ...
        if (message.getType() == ObserverMessage.NETWORK_STATUS || (int) message.getValue() == Controller.STATUS_OK) {
            ...
            if (((Transaction) message.getValue()).getType() == Transaction.SEND_ASSET_TRANSACTION) {
                ...
                if (!is) {
                    addMessage(0, (RSend) message.getValue(), null, incoming);
    ...
    private void addMessage(int pos, RSend transaction, Account account, Boolean incoming) {
    ...
    public class MessageBuf {
        ...
        public MessageBuf(byte[] rawMessage, boolean encrypted, PublicKeyAccount sender, Account recipient, long timestamp, BigDecimal amount, long assetKey, BigDecimal fee, byte[] signature, byte[] senderPublicKey, boolean isText, Account account1, boolean incoming1) {
                ...
                if (key < 0) {
                    ...
                    if (amo_sign < 0) {
                        ...
                        if (sender.equals(account1)) {
                            send_type = Lang.getInstance().translate("Sent");
                        } else {
                            send_type = Lang.getInstance().translate("Received");
                        }
                    }
                }
