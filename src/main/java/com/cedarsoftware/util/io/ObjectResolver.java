...
public class ObjectResolver extends Resolver {
    ...
    protected void assignField(final Deque<JsonObject<String, Object>> stack, final JsonObject jsonObj,
        ...
        try {
            ...
            if (rhs instanceof JsonObject) {
                if (field.getGenericType() instanceof ParameterizedType) {   // Only JsonObject instances could contain unmarked objects.
            ...
            if (rhs == JsonParser.EMPTY_OBJECT) {
                ...
                if (char[].class == fieldType) {   // Specially handle char[] because we are writing these
                ...
                } else if (rhs instanceof String && "".equals(((String) rhs).trim()) && fieldType != String.class) {   // Allow "" to null out a non-String field
                    field.set(target, null);
                } else {
    ...
    protected void traverseCollection(final Deque<JsonObject<String, Object>> stack, final JsonObject<String, Object> jsonObj) {
        ...
        for (final Object element : items) {
            ...
            if (element == null) {
                col.add(null);
            } else if (element == JsonParser.EMPTY_OBJECT) {   // Handles {}
    ...
    protected void traverseArray(final Deque<JsonObject<String, Object>> stack, final JsonObject<String, Object> jsonObj) {
        ...
        if (byte.class == compType) {   // Handle byte[] special for performance boost.
        ...
        for (int i = 0; i < len; i++) {
            ...
            if (element == null) {
            ...
            } else if (element == JsonParser.EMPTY_OBJECT) {    // Use either explicitly defined type in ObjectMap associated to JSON, or array component type.
                ...
                if (ref != null) {    // Connect reference
                    JsonObject refObject = getReferencedObj(ref);
                    if (refObject.target != null) {   // Array element with reference to existing object
                    ...
                    Array.set(array, i, arrayElement);
                    if (!MetaUtils.isLogicalPrimitive(arrayElement.getClass())) {    // Skip walking primitives, primitive wrapper classes, Strings, and Classes
                        stack.addFirst(jsonObject);
                    }
                }
            } else {
                if (element instanceof String && "".equals(((String) element).trim()) && compType != String.class && compType != Object.class) {   // Allow an entry of "" in the array to set the array element to null, *if* the array type is NOT String[] and NOT Object[]
                    Array.set(array, i, null);
                } else {
