...
public abstract class AssetCls extends ItemCls {
    ...
    public static String viewAssetTypeDescriptionCls(int asset_type) {
        switch (asset_type) {
            ...
            case AS_OUTSIDE_OTHER_CLAIM:
                return "Другие внешние права, требования и обязательства. Любое обязательство (как и другие внешние активы), которое может быть истребовано внесением записи \"требования исполнения\" и погашено записью \"подтверждения исполнения\"";

            case AS_INSIDE_ASSETS:
                return "Внутренний (цифровой) актив. То что не требует вовне каких либо дополнительных действий при передаче его между счетами внутри Эрачейн";
            case AS_INSIDE_CURRENCY:
            ...
            case AS_INSIDE_SHARE:
                return "Цифровая акция. Доля собственности на внешнее или внутренне предприятие, обладание которой устанавливает право на владение соотвествующей долей на предприятие вовне без надобности совершать какие-либо действия";
            case AS_INSIDE_BONUS:
                return "Цифровая награда (бонус). То что не имеет общепринятой стомости и не может обмениваться на другие виды активов внутри Эрачейн. Хотя обмен на другие бонусы и награды разрешен";
            case AS_INSIDE_RIGHTS:
