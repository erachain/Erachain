...
public abstract class AssetCls extends ItemCls {
    ...
    public static String viewAssetTypeDescriptionCls(int asset_type) {
        switch (asset_type) {
            case AS_OUTSIDE_GOODS:
                return "Движимые вещи и товары. Эти товары могут быть взяты на хранение кладовщиком или доставку доставщиком. При этом видны остатки на счетах кладовщиков и доставщиков";
            case AS_OUTSIDE_IMMOVABLE:
                return "Недвижимость и иные товары и вещи не подлежащие доставке. Такие вещи можно взять и дать в аренду";
                
            case AS_OUTSIDE_CURRENCY:
                return "Внешние деньги, которые необходимо перевести на внешний банковский счет или передать наличными. Величина на вашем счете показывает право требовать к эмитенту перевод такого количества денег на ваш банковский счет. Для удовлетворения требования необходимо выставит требование на полату, а после прихода денег подтвердить погашение этого требования";
            case AS_OUTSIDE_SERVICE:
                return "Внешняя услуга, которую необходимо оказать вовне. Для уведомления вашего желания на оказание услуги необходимо предъявить требование и потом подтвердить ее исполнение для вас";
            case AS_OUTSIDE_SHARE:
            ...
            case AS_OUTSIDE_OTHER_CLAIM:
                return "Другие внешние права, требования и обязательства. Любое обязательство (как и другие внешние активы), которое может быть истребовано внесением записи \"требования исполнения\" и погашено записью \"подтверждения исполнения\" оного";

            case AS_INSIDE_ASSETS:
                return "Внутренний (цифровой) актив. То что не требует вовне каких либо дополнительных действий при передаче его между счетами внутри Эрасчейн";
            case AS_INSIDE_CURRENCY:
                return "Цифровые деньги";
            case AS_INSIDE_UTILITY:
                return "Цифровая услуга или стоимость - то что может быть использовано внутри Эрачейн, например как оплтат за внешние услуги";
            case AS_INSIDE_SHARE:
                return "Цифровая акция. Доля собственности на внешнее или внутренне предприятие, Обладание котрой устанавливает право на владение соотвествующей долей на предприятие вовне без надобности совершать какие-либо действия";
            case AS_INSIDE_BONUS:
                return "Цифровая награда (бонус). То что не имеет общепринятой стомости и не может обмениваться на другие виды активов внутри Эрачейн. Хотя обмен на тдругие бонусы и награды разрешен";
            case AS_INSIDE_RIGHTS:
            ...
            case AS_ACCOUNTING:
                return "Счетные (бухгалтерские) единицы. Не имеют общепринятой стоимости и могут обмениваться только на между собой. Все действия с ними возможны в целях учета - передать, принять на хранение (на баланс), выдать долг, забрать долг и т.д.";
        }
        return "";
    }
