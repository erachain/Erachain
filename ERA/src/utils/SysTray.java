...
public class SysTray implements Observer{
		...
		if (icon == null) {
			if (!SystemTray.isSupported()) {
				...
				TrayIcon icon = new TrayIcon(createImage(
						"images/icons/icon32.png", "tray icon"), "ARONICLE.com "
	...
	private PopupMenu createPopupMenu() throws HeadlessException {
		...
		MenuItem decentralizedWeb = new MenuItem("ARONICLE.com Web/Social Network");
		...
		messages.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent e) {
				    ...
				    frame.setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);
				    frame.setVisible(true);
					Send_Panel ap = new Send_Panel(null, null);
					frame.getContentPane().add(ap);
	...
	public void update(Observable arg0, Object arg1) {
		...
		if(message.getType() == ObserverMessage.WALLET_SYNC_STATUS)	{
			...
			toolTipText += "\nHeight: " + currentHeight + "/" + Controller.getInstance().getMyHWeight(false).a + "/" + Controller.getInstance().getMaxPeerHWeight(false).a;
			...
			toolTipText += networkStatus + " " + syncProcent;
			toolTipText += "\nHeight: " + currentHeight + "/" + Controller.getInstance().getMaxPeerHWeight(false);
			setToolTipText(toolTipText);
