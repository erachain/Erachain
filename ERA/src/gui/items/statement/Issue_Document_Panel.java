...
public class Issue_Document_Panel extends javax.swing.JPanel {
	...
	{
		// check title
		if (exData_Panel.jTextField_Title_Message.getText()=="" || exData_Panel.jTextField_Title_Message.getText().length() < 5){
		...
		catch(Exception e)
		{
			...
			case 5:
				...
				JOptionPane.showMessageDialog(new JFrame(), Lang.getInstance().translate("Template not exist!"), Lang.getInstance().translate("Error"), JOptionPane.ERROR_MESSAGE);
				break;
			}
			return null;
		}
		...
		R_SignNote issueDoc = (R_SignNote) Controller.getInstance().r_SignNote(
				...
				feePow, key, messageBytes,new byte[]{1}, new byte[]{0});
