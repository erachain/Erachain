...
public class OrderPanel extends JPanel
{
	...
	public void calculateBuyingPrice(JTextField target, boolean buying)
	{
	    try
	    {
	    	...
	    	target.setText(BigDecimal.ONE.setScale(BlockChain.AMOUNT_DEDAULT_SCALE).divide(price, RoundingMode.DOWN).toPlainString());
	    }
	    catch(Exception e)
	    {
	    	target.setText("0");
	...
	public void onSellClick(boolean buying)
	{
		...
		try
		{
			...
			//READ AMOUNT
			parse = 1;
			amountHave = new BigDecimal(this.txtAmount.getText()).setScale(BlockChain.AMOUNT_DEDAULT_SCALE);

			//READ PRICE
			parse = 2;
			price = new BigDecimal(this.txtPrice.getText()).setScale(BlockChain.AMOUNT_DEDAULT_SCALE);
		...
		Transaction transaction = Controller.getInstance().createOrder(creator, this.have, this.want, amountHave.setScale(BlockChain.AMOUNT_DEDAULT_SCALE, RoundingMode.HALF_DOWN), amountWant.setScale(8, RoundingMode.HALF_DOWN), feePow);
