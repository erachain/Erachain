...
public class R_Vouch extends Transaction {
	...
	public int isValid(DBSet db, Long releaserReference) {
		...
		if (!this.creator.isPerson(db))
			if (this.creator.getConfirmedBalance(Transaction.RIGHTS_KEY, db).compareTo(BigDecimal.TEN) <= 0)
	...
	public void process(DBSet db, boolean asPack) {
		...
		BigDecimal amount = this.creator.getConfirmedBalance(Transaction.RIGHTS_KEY, db);
	...
	public void orphan(DBSet db, boolean asPack) {
		...
		Tuple2<BigDecimal, List<Tuple2<Integer, Integer>>> valueNew =
				new Tuple2<BigDecimal, List<Tuple2<Integer, Integer>>>(
					value.a.subtract(this.creator.getConfirmedBalance(Transaction.RIGHTS_KEY, db)),
					listNew
					);
		db.getVouchRecordMap().set(recordKey, valueNew);
