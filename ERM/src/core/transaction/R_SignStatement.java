...
public class R_SignStatement extends Transaction {
	...
	public R_SignStatement(byte[] typeBytes, PublicKeyAccount creator, byte feePow, long noteKey, byte[] data, byte[] isText, byte[] encrypted, long timestamp, Long reference) {
	...
	public R_SignStatement(byte[] typeBytes, PublicKeyAccount creator, byte feePow, long noteKey, byte[] data, byte[] isText, byte[] encrypted, long timestamp, Long reference, byte[] signature) {
		this(typeBytes, creator, feePow, noteKey, data, isText, encrypted, timestamp, reference);
		this.signature = signature;
		this.calcFee();
	}
	// asPack
	public R_SignStatement(byte[] typeBytes, PublicKeyAccount creator, long noteKey, byte[] data, byte[] isText, byte[] encrypted, Long reference, byte[] signature) {
		this(typeBytes, creator, (byte)0, noteKey, data, isText, encrypted, 0l, reference);
		this.signature = signature;
		// not need this.calcFee();
	}
	public R_SignStatement(PublicKeyAccount creator, byte feePow, long noteKey, byte[] data, byte[] isText, byte[] encrypted, long timestamp, Long reference, byte[] signature) {
		this(new byte[]{TYPE_ID, 0, 0, 0}, creator, feePow, noteKey, data, isText, encrypted, timestamp, reference, signature);
		// set props
		this.setTypeBytes();
	}
	public R_SignStatement(PublicKeyAccount creator, byte feePow, long noteKey, byte[] data, byte[] isText, byte[] encrypted, long timestamp, Long reference) {
		this(new byte[]{TYPE_ID, 0, 0, 0}, creator, feePow, noteKey, data, isText, encrypted, timestamp, reference);
		// set props
		this.setTypeBytes();
	}
	public R_SignStatement(byte[] typeBytes, PublicKeyAccount creator, byte feePow, long noteKey, byte[] data,
	...
	// as Pack
	public R_SignStatement(byte[] typeBytes, PublicKeyAccount creator, long noteKey, byte[] data,
	...
	public R_SignStatement(byte prop1, byte prop2, byte prop3, PublicKeyAccount creator, byte feePow, long noteKey, byte[] data, byte[] isText, byte[] encrypted, long timestamp, Long reference)
	{
	...
	public static Transaction Parse(byte[] data, Long releaserReference) throws Exception
	{
		...
		if (signersLen == 0) {
			if (!asPack) {
				return new R_SignStatement(typeBytes, creator, feePow, key, arbitraryData, isTextByte, encryptedByte, timestamp, reference, signatureBytes);
			} else {
				return new R_SignStatement(typeBytes, creator, key, arbitraryData, isTextByte, encryptedByte, reference, signatureBytes);
			}
		} else {
			if (!asPack) {
				return new R_SignStatement(typeBytes, creator, feePow, key, arbitraryData, isTextByte, encryptedByte, signers, signatures, timestamp, reference, signatureBytes);
			} else {
				return new R_SignStatement(typeBytes, creator, key, arbitraryData, isTextByte, encryptedByte, signers, signatures, reference, signatureBytes);
			}
