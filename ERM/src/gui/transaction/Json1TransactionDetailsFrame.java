...
public class Json1TransactionDetailsFrame extends JFrame
{
	...
	public Json1TransactionDetailsFrame(final JsonTransaction json1Transaction)
	{
		...
		//SIGNATURE
		detailGBC.gridy = componentLevel;
		JTextField signature = new JTextField(Base58.encode(json1Transaction.getSignature()));
		...
		//REFERENCE
		detailGBC.gridy = componentLevel;
		JTextField reference = new JTextField(Base58.encode(json1Transaction.getReference()));
		...
		//TIMESTAMP
		detailGBC.gridy = componentLevel;
		JTextField timestamp = new JTextField(DateTimeFormat.timestamptoString(json1Transaction.getTimestamp()));
		...
		//SENDER
		detailGBC.gridy = componentLevel;
		JTextField sender = new JTextField(json1Transaction.getCreator().getAddress());
		...
		//RECIPIENT
		detailGBC.gridy = componentLevel;
		JTextField recipient = new JTextField(json1Transaction.getRecipient().getAddress());
        ...
        encrypted.setSelected(json1Transaction.isEncrypted());
        encrypted.setEnabled(json1Transaction.isEncrypted());
        ...
        encrypted.addActionListener(new ActionListener()
        {
        	public void actionPerformed(ActionEvent e)
        	{
        		if(!encrypted.isSelected())
        		{
	        		...
	        		if(account == null)
	        		{
	            		PrivateKeyAccount accountRecipient = Controller.getInstance().getPrivateKeyAccountByAddress(json1Transaction.getRecipient().getAddress());
	    				privateKey = accountRecipient.getPrivateKey();		
	    				
	    				publicKey = json1Transaction.getCreator().getPublicKey();    				
	        		}
	        		//IF SENDER ME
	        		else
	        		{
	    				...
	    				publicKey = Controller.getInstance().getPublicKeyByAddress(json1Transaction.getRecipient().getAddress());    				
	        		}
	        		
	        		try {
	        			messageText.setText(new String(AEScrypto.dataDecrypt(json1Transaction.getData(), privateKey, publicKey), "UTF-8"));
        		...
        		{
        			try {
        				messageText.setText(new String(json1Transaction.getData(), "UTF-8"));
					} catch (UnsupportedEncodingException e1) {
						// TODO Auto-generated catch block
						e1.printStackTrace();
					}
		...
		detailGBC.gridy = componentLevel;
		detailGBC.gridwidth = 2;
		JTextField amount = new JTextField(json1Transaction.getAmount().toPlainString());
		...
		JTextField asset = new JTextField(Controller.getInstance().getAsset( json1Transaction.getKey()).toString());
		...
		//FEE
		detailGBC.gridy = componentLevel;
		JTextField fee = new JTextField(json1Transaction.getFee().toPlainString());
		...
		//CONFIRMATIONS
		detailGBC.gridy = componentLevel;
		JLabel confirmations = new JLabel(String.valueOf(json1Transaction.getConfirmations()));
