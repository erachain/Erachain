...
public class UnionConfirmFrame extends JFrame  {

	private JComboBox<Account> accountLBox;

	public UnionConfirmFrame(JComponent  apers, UnionCls union) {
		super();
...
//		setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);
	    ...
	    input.gridx = 0;
	    this.add(new JLabel(Lang.getInstance().translate("Public Keys of") + " " + union.getName() +":"), input);
...
/*
	...
	private void refreshReceiverDetails(JTextField pubKeyTxt, JLabel pubKeyDetails)
	{
		...
		if (!isValid) {
			...
			String unionDetails;
			Tuple4<Long, Integer, Integer, byte[]> addressDuration = null; //account.getUnionDuration(DBSet.getInstance());
			
			if (addressDuration == null) unionDetails = Lang.getInstance().translate("Not unionalized yet");
			else
			{
