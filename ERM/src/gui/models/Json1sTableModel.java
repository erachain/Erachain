...
public class Json1sTableModel extends JTable implements Observer{
	...
	public Json1sTableModel()
	{
		...
		for (Account account : Controller.getInstance().getAccounts()) {
			transactions.addAll(DBSet.getInstance().getTransactionFinalMap().getTransactionsByTypeAndAddress(account.getAddress(), Transaction.JSON_TRANSACTION, 0));	
		}
	...
	public synchronized void syncUpdate(Observable o, Object arg)
	{
		...
		{		
			boolean is;
			if(((Transaction) message.getValue()).getType() == Transaction.JSON_TRANSACTION)
			{
				is = false;
