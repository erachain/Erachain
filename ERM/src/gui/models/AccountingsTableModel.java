...
public class AccountingsTableModel extends JTable implements Observer{
	...
	public AccountingsTableModel()
	{
		...
		for (Account account : Controller.getInstance().getAccounts()) {
			transactions.addAll(DBSet.getInstance().getTransactionFinalMap().getTransactionsByTypeAndAddress(account.getAddress(), Transaction.ACCOUNTING_TRANSACTION, 0));	
		}
		...
		for (Transaction messagetx : transactions) {
			...
			if(!is)
			{
				addMessage(messageBufs.size(),(AccountingTransaction)messagetx);
			} 
		}
		...
		copyMessage.addActionListener(new ActionListener()
		{
			public void actionPerformed(ActionEvent e) 
			{
		        ...
		        AccountingsTableModel invokerAsJComponent = (AccountingsTableModel) invoker;
        ...
        this.addMouseListener(new MouseAdapter() 
			{
				public void mouseClicked(MouseEvent e) 
				{
					...
					{
						AccountingsTableModel tableModelparent = (AccountingsTableModel) e.getSource();
	...
	public synchronized void syncUpdate(Observable o, Object arg)
	{
		...
		{		
			boolean is;
			if(((Transaction) message.getValue()).getType() == Transaction.ACCOUNTING_TRANSACTION)
			{
				is = false;
