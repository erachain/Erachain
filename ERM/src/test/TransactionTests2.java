...
public class TransactionTests2 {
	...
	public void validateAccountingTransaction() 
	{
		...
		Asset asset = new Venture(maker, "ERM1", "1It is the basic unit of Environment Real Management", 100L, (byte) 6, true);
		IssueAssetTransaction iat = new IssueAssetTransaction(maker, asset, FEE_POWER, timestamp, maker.getLastReference(db));
		...
		AccountingTransaction accountingTransaction = new AccountingTransaction(
				...
				new byte[] { 0 },
				timestamp,
				maker.getLastReference(db)
				);
		accountingTransaction.sign(maker);
		...
		{	
			...
			//CHECK AMOUNT SENDER
			assertEquals(accountingTransaction.viewAmount(maker), accountingTransaction_parsed.viewAmount(maker));	
		...
		assertEquals(BigDecimal.valueOf(77).setScale(8), maker.getConfirmedBalance(key, db));
		assertEquals(BigDecimal.valueOf(23).setScale(8), recipient.getConfirmedBalance(key, db));
